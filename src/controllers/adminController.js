...
async function getPendingVerifications(req, res) {
	try {
		...
		throw new AppError(error.message || "Failed to retrieve pending verifications", 500);
	}
}
...
async function updateVerificationStatus(req, res) {
	try {
		...
		if (status !== 'Verified' && status !== 'Rejected') {
			throw new AppError("Invalid status provided. Must be 'Verified' or 'Rejected'", 400);
		}
		...
		throw new AppError(error.message || "Failed to update user verification status", 500);
	}
}
...
async function getAdminSummary(req, res) {
	try {
		...
		throw new AppError(error.message || "Failed to load admin summary data", 500);
	}
}
